...
@php
    ...
    // Valores guardados (con fallback a defaults)
    $saved = [
...
<div class="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
    ...
    <form method="POST" action="{{ route('dashboard.branches.qr.save', $branch) }}"
          x-data="{ open: null }">
        @csrf
                    ...
                    <div class="grid grid-cols-5 gap-2 pt-4">
                        ...
                        <label class="scheme-option cursor-pointer group" title="{{ $s['label'] }}">
                            ...
                            <div class="w-full aspect-square rounded-lg border-2 border-transparent
                                        group-hover:border-slate-400 transition scheme-swatch
                ...
                <div x-show="open === 'texto'" x-transition class="px-5 pb-5 border-t border-slate-100">
                    <div class="space-y-3 pt-4">
                        <div>
                            ...
                            <input type="text" id="input-headline" name="qr_headline"
                        ...
                        <div>
                            ...
                            <textarea id="input-instruction" name="qr_instruction" rows="2" maxlength="200"
                                      oninput="schedulePreviewUpdate()"
                ...
                <div x-show="open === 'visibilidad'" x-transition class="px-5 pb-5 border-t border-slate-100">
                    <div class="space-y-4 pt-4">
                        ...
                        <label class="flex items-center justify-between cursor-pointer">
                            ...
                            <div class="relative">
                                <input type="checkbox" id="toggle-logo" name="qr_show_logo" value="1"
                                       class="sr-only"
                        ...
                        <label class="flex items-center justify-between cursor-pointer">
                            ...
                            <div class="relative">
                                <input type="checkbox" id="toggle-branch" name="qr_show_branch" value="1"
                                       class="sr-only"
...
(function () {
    ...
    // Aplicar layout inicial si es A4
    @if($saved['layout'] === 'a4')
    (function() {
        const l = LAYOUTS.a4;
    ...
    function buildParams(isPreview) {
        return new URLSearchParams({
            scheme:        document.querySelector('[name="qr_scheme"]:checked')?.value         ?? 'blue',
            layout:        document.querySelector('[name="qr_layout"]:checked')?.value         ?? 'a5',
            qr_size:       document.querySelector('[name="qr_qr_size"]:checked')?.value        ?? 'md',
            headline_size: document.querySelector('[name="qr_headline_size"]:checked')?.value  ?? 'md',
            instr_size:    document.querySelector('[name="qr_instr_size"]:checked')?.value     ?? 'md',
            logo_size:     document.querySelector('[name="qr_logo_size"]:checked')?.value      ?? 'md',
            logo_position: document.querySelector('[name="qr_logo_position"]:checked')?.value  ?? 'center',
    ...
    window.schedulePreviewUpdate = function () {
        ...
        debounce = setTimeout(function () {
            const selected = document.querySelector('[name="qr_scheme"]:checked')?.value;
